import { useSelector } from "react-redux";

const Bagsummary = () =>{
  const bagItems = useSelector(store => store.bag);
  const Convenience_Fee = 99;
  let totalItems = bagItems.length;
  let totalMRP=0;
  let totalDiscount = 0;
  const items = useSelector(store => store.items);
  const finalItems = items.filter(val => {
    const index = bagItems.indexOf(val.id);
    return index>=0;
  })

  finalItems.map((bagSample) =>{
    totalMRP+=bagSample.original_price;
    totalDiscount+=bagSample.original_price - bagSample.current_price;
  })
  let finalPayment = totalMRP - totalDiscount + Convenience_Fee;

  return(
  <div className="bag-summary">
   <div className="bag-details-container">
  <div className="price-header">PRICE DETAILS ({totalItems} Items) </div>
  <div className="price-item">
    <span className="price-item-tag">Total MRP</span>
    <span className="price-item-value">₹{totalMRP}</span>
  </div>
  <div className="price-item">
    <span className="price-item-tag">Discount on MRP</span>
    <span className="price-item-value priceDetail-base-discount">-₹{totalDiscount}</span>
  </div>
  <div className="price-item">
    <span className="price-item-tag">Convenience Fee</span>
    <span className="price-item-value">₹{Convenience_Fee}</span>
  </div>
  <hr/>
  <div className="price-footer">
    <span className="price-item-tag">Total Amount</span>
    <span className="price-item-value">₹{finalPayment}</span>
  </div>
</div>
<button className="btn-place-order">
  <div className="css-xjhrni">PLACE ORDER</div>
</button>
</div>
  );
}

export default Bagsummary;